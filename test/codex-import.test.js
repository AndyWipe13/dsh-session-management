...
import { setupSessionService } from './helpers/fake-services.js'
import { convertCodexRecords, createCodexImportAdapter, readCodexFile, resolveCodexTitle, summarizeCodexRecords } from '../lib/codex.js'
...
async function importCodex(service, selections, root) {
  const scan = await service.scan('codex', root)
  return service.import('codex', scan.scanId, selections)
}
...
test('Codex scan returns only unimported main sessions with real user messages', async () => {
  const { manifest, service } = setupSessionService({ imports: [createCodexImportAdapter()] })
  const result = await service.scan('codex', codexFixtures)
...
test('Codex import counts bad lines in the report while still importing the session', async () => {
  ...
  const { manifest, service } = setupSessionService({ imports: [createCodexImportAdapter()] })
  try {
    const report = await importCodex(service, [{ sourceSessionId: 'codex-with-bad-line' }], dir)
...
test('session_index.jsonl thread title beats first user text when different', async () => {
  ...
  const { manifest, service } = setupSessionService({ imports: [createCodexImportAdapter()] })
  try {
    const result = await service.scan('codex', dir)
    assert.equal(result.total, 1)
...
test('convertCodexRecords opens one DSH turn per real user prompt', () => {
  const records = [
    {
      value: {
        type: 'response_item',
  ...
  const result = convertCodexRecords(records, { knowTool: () => false })
  const turnStarts = result.events.filter((event) => event.type === 'turn/start')
  const assistantTurns = result.events
    .filter((event) => event.type === 'assistant/message')
    .map((event) => event.data.turn)

  assert.deepEqual(turnStarts.map((event) => event.data.turn), [1, 2])
  assert.deepEqual(assistantTurns, [1, 2])
...
test('Codex import writes through the official session seed path and records manifest', async () => {
  const { ctx, manifest, service } = setupSessionService({ imports: [createCodexImportAdapter()] })
  const report = await importCodex(service, [
...
test('Codex import skips already imported sessions', async () => {
  const { manifest, service } = setupSessionService({ imports: [createCodexImportAdapter()] })
  const scan = await service.scan('codex', codexFixtures)
  const first = await service.import('codex', scan.scanId, [
    { sourceSessionId: '00000000-0000-0000-0000-0000004c' },
  ])
  assert.equal(first.success, 1)

  const second = await service.import('codex', scan.scanId, [
    { sourceSessionId: '00000000-0000-0000-0000-0000004c' },
  ])
...
test('Codex import reports failed target when session is not in scan queue', async () => {
  ...
  assert.match(report.items[0].reason, /not in this import queue snapshot/)
  await manifest.close()
})

test('Codex source files remain untouched by scan and import', async () => {
  const { manifest, service } = setupSessionService({ imports: [createCodexImportAdapter()] })
  await service.scan('codex', codexFixtures)
  await importCodex(service, [{ sourceSessionId: '00000000-0000-0000-0000-0000004c' }], codexFixtures)
