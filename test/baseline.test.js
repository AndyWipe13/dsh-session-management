...
test('plugin baseline exports shape', () => {
  assert.equal(name, '@nathan110628/dsh-session-management')
  assert.deepEqual(inject, ['tools', 'sessions', 'agents', 'apiProxy', 'sessionQuery', 'sessionPersistence', 'workspaceRegistry', 'storageDomain', 'webServer'])
...
test('bundle patch points at the built plugin module', () => {
  ...
  assert.match(patch, /@nathan110628\/dsh-session-management/)
...
test('plugin applies cleanly and registers session tools and settings api', () => {
  ...
  assert.ok(
    ctx.$registeredRoutes.some((route) => route.kind === 'prefix' && route.path.includes('/@nathan110628/dsh-session-management/api')),
