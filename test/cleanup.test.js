...
test('stats and cleanup share metrics cached by persistence revision', async () => {
  const { ctx, manifest, service } = setupSessionService()
  const records = [
    { header: { id: 's1', createdAt: 1000, cwd: 'C:/work' }, live: false, persisted: true, blank: false },
    { header: { id: 's2', createdAt: 2000, cwd: 'C:/work' }, live: false, persisted: true, blank: false },
  ]
  const revisions = new Map([['s1', 'r1'], ['s2', 'r1']])
  let eventReads = 0
  seedSessions(ctx, records, {
    events: {
      s1: [{ type: 'turn/start', time: 1000 }, { type: 'user/message', time: 1001 }],
      s2: [{ type: 'turn/start', time: 2000 }, { type: 'user/message', time: 2001 }],
    },
  })
  ctx.sessionQuery.listEvents = async (id) => {
    eventReads += 1
    return id === 's1'
      ? [{ type: 'turn/start', time: 1000 }, { type: 'user/message', time: 1001 }]
      : [{ type: 'turn/start', time: 2000 }, { type: 'user/message', time: 2001 }]
  }
  ctx.sessionPersistence.list = async () => records.map((record) => ({
    header: record.header,
    revision: revisions.get(record.header.id),
    sizeBytes: 100,
  }))

  await service.stats()
  await service.stats()
  await service.cleanupPreview({ olderThanDays: 0, largerThanMb: 0, emptySessions: true, archivedOnly: false })
...
test('stats falls back to persistence stat when bulk list has no snapshots', async () => {
  const { ctx, manifest, service } = setupSessionService()
  const record = { header: { id: 's1', createdAt: 1000, cwd: 'C:/work' }, live: false, persisted: true, blank: false }
  let eventReads = 0
  let titleReads = 0
  let statReads = 0
  seedSessions(ctx, [record])
  ctx.sessionPersistence.list = async () => []
  ctx.sessionPersistence.stat = async () => {
    statReads += 1
    return { header: record.header, revision: 'r1', sizeBytes: 100 }
  }
  ctx.sessionQuery.readTitleSnapshots = async (ids) => {
    titleReads += 1
    return ids.map((sessionId) => ({ sessionId, status: 'fulfilled', value: { title: 'Cached' } }))
  }
  ctx.sessionQuery.listEvents = async () => {
    eventReads += 1
    return [{ type: 'turn/start', time: 1000 }]
  }
...
test('stats falls back to read-only artifact identity on older hosts', async () => {
  let artifactStats = 0
  const { ctx, manifest, service } = setupSessionService({
    sessionArtifactStat: async () => {
      artifactStats += 1
      return { sizeBytes: 100, mtimeMs: 1234 }
    },
  })
  const record = { header: { id: 's1', createdAt: 1000, cwd: 'C:/work' }, live: false, persisted: true, blank: false }
  let eventReads = 0
  ...
  seedSessions(ctx, [record])
  ctx.sessionPersistence.list = async () => []
  ctx.sessionPersistence.stat = undefined
  ctx.sessionPersistence.locate = () => ({ path: 'C:/sessions/s1/session.jsonl' })
  ctx.sessionQuery.readTitleSnapshots = async (ids) => {
    titleReads += 1
    return ids.map((sessionId) => ({ sessionId, status: 'fulfilled', value: { title: 'Cached' } }))
  }
  ...
  await service.stats()
  await service.stats()
  assert.equal(artifactStats, 2)
  assert.equal(titleReads, 1)
  assert.equal(eventReads, 1)
  await manifest.close()
})

test('stats caches live metrics by the official session sequence', async () => {
  const { ctx, manifest, service } = setupSessionService()
  const record = { header: { id: 'live', createdAt: 1000, cwd: 'C:/work' }, live: true, persisted: true, blank: false }
  let seq = 2
  let eventReads = 0
  let titleReads = 0
  const snapshotStarts = []
  const events = [
    { type: 'turn/start', time: 1000 },
    { type: 'user/message', time: 1001 },
    { type: 'assistant/message', time: 1002 },
  ]
  seedSessions(ctx, [record])
  ctx.sessionQuery.readTitleSnapshots = async (ids) => {
    titleReads += 1
    return ids.map((sessionId) => ({ sessionId, status: 'fulfilled', value: { title: 'Live title' } }))
  }
  ctx.sessions.get = async () => ({
    id: 'live',
    seq,
    snapshotEvents: (from = 0, to = seq) => {
      snapshotStarts.push(from)
  ...
  seq = 3
  const changed = await service.stats()
  assert.equal(eventReads, 0)
  assert.equal(titleReads, 1)
  assert.deepEqual(snapshotStarts, [0, 2, 2])
  assert.equal(changed.sessions[0].messageCount, 2)
  await manifest.close()
})
...
test('cleanupExecute requires DELETE token and a live preview before any side effect', async () => {
  const { ctx, manifest, service, deletedPaths } = setupSessionService()
  seedSessions(ctx, [{ header: { id: 's1', createdAt: 1000, cwd: 'C:/work' }, live: false, persisted: true, blank: false }], {
