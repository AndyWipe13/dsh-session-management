...
import { createClaudeImportAdapter, convertClaudeRecords, readClaudeFile, summarizeClaudeRecords } from '../lib/claude.js'
...
async function importClaude(service, selections, root) {
  const scan = await service.scan('claude-code', root)
  return service.import('claude-code', scan.scanId, selections)
}

test('Claude scan returns only unimported main sessions with real user messages', async () => {
  const { manifest, service } = setupSessionService({ imports: [createClaudeImportAdapter()] })
  const result = await service.scan('claude-code', claudeFixtures)
...
test('Claude import counts bad lines in the report while still importing the session', async () => {
  ...
  const { manifest, service } = setupSessionService({ imports: [createClaudeImportAdapter()] })
  try {
    const report = await importClaude(service, [{ sourceSessionId: 'session-with-bad-line' }], dir)
...
test('Claude import writes through the official session seed path and records manifest', async () => {
  const { ctx, manifest, service } = setupSessionService({ imports: [createClaudeImportAdapter()] })
  const report = await importClaude(service, [
...
test('Claude import skips already imported sessions', async () => {
  const { manifest, service } = setupSessionService({ imports: [createClaudeImportAdapter()] })
  const scan = await service.scan('claude-code', claudeFixtures)
  const first = await service.import('claude-code', scan.scanId, [
    { sourceSessionId: '00000000-0000-0000-0000-00000000' },
  ])
  assert.equal(first.success, 1)

  const second = await service.import('claude-code', scan.scanId, [
    { sourceSessionId: '00000000-0000-0000-0000-00000000' },
  ])
...
test('Claude source files remain untouched by scan and import', async () => {
  const { manifest, service } = setupSessionService({ imports: [createClaudeImportAdapter()] })
  await service.scan('claude-code', claudeFixtures)
  await importClaude(service, [{ sourceSessionId: '00000000-0000-0000-0000-00000000' }], claudeFixtures)
