...
export class SessionManagementService {
    ...
    async list(filters = {}) {
        ...
        const mapped = await this.mapConcurrent(records, 16, async (raw) => {
    ...
    async searchContent(query, filters = {}) {
        ...
        const archived = archivedSetOf(this.ctx.workspaceRegistry);
        const mapped = await this.mapConcurrent(hits, 16, async (hit) => {
    ...
    async scanCodexFiles(root, includeImported = false) {
        const reader = this.requireCodex();
        const scanRoot = root ?? this.options.codexPath;
        if (!scanRoot) {
            throw new Error('Codex import root is not configured; pass codexPath or scan root');
    ...
    async scanClaudeFiles(root, includeImported = false) {
        const reader = this.requireClaude();
        const scanRoot = root ?? this.options.claudePath;
        if (!scanRoot) {
            throw new Error('Claude import root is not configured; pass claudePath or scan root');
        ...
        for (const raw of await this.ctx.sessionQuery.listSessions()) {
            ...
            try {
                ...
                if (!await this.titleOf(id)) {
                    ...
                    if (candidate?.title)
                        await this.restoreImportedTitle(id, candidate.title);
                }
    ...
    async restoreImportedTitle(id, title) {
        ...
        const live = await sessions.get?.(id);
        if (live) {
    ...
    async createImportedSession(conversion, title) {
        const sessions = this.ctx.sessions;
        if (!sessions ||
    ...
    computeMetrics(events) {
        ...
        for (const event of events) {
            ...
            if (type === 'user/message' || type === 'assistant/message')
                ...
                toolCalls++;
            else if (type === 'tool/result' && this.isToolResultSuccess(event))
                toolSuccess++;
            if (type === 'turn/start')
                hasTurnStart = true;
