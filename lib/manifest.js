...
export const SESSION_SOURCES = ['dsh', 'claude-code', 'codex'];
...
const importRecordSchema = {
    parse(value) {
        ...
        const v = value;
        if (!SESSION_SOURCES.includes(v.source)) {
...
export function openManifestStore(storageDomain) {
    ...
    return {
        ...
        async removeBySource(source, sourceSessionId) {
            const sourceKey = `source:${source}:${sourceSessionId}`;
            const record = (await read(sourceKey));
            if (!record)
                return;
