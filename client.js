/**
 * Client half for @nathan110628/dsh-session-management.
 *
 ...
 * `/@nathan110628/dsh-session-management/api`.
 *
 * Design notes:
...
window.__ModuleLoader__.load({
  id: '@nathan110628/dsh-session-management',
  factory: (require) => {
...
/* ---------- header & tabs ---------- */
.dshsm-head{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:2px}
...
.dshsm-head .dshsm-view-toggle{margin-left:auto}
.dshsm-settings-panel-managed[role="dialog"]{transition:width .2s,height .2s,max-width .2s,border-radius .2s}
.dshsm-settings-panel-managed.dshsm-settings-panel-maximized[role="dialog"]{width:calc(100vw - 16px)!important;height:calc(100vh - 16px)!important;max-width:none!important;border-radius:20px!important}
...
.dshsm-load-more{height:38px;display:flex;align-items:center;justify-content:center;gap:7px;border-top:1px solid var(--dshsm-border);color:var(--dshsm-text-3);font-size:12px;background:var(--dshsm-surface)}
.dshsm-load-more:empty{height:1px;border-top:0}
...
.dshsm-list td{padding:10px 12px;border-bottom:1px solid var(--dshsm-border);vertical-align:middle;text-align:left}
...
.dshsm-num,.dshsm-list th.dshsm-num{text-align:left;font-family:var(--dshsm-mono);font-size:12px;font-variant-numeric:tabular-nums;white-space:nowrap}
.dshsm-size,.dshsm-list th.dshsm-size{font-family:var(--dshsm-mono);font-size:12px;font-variant-numeric:tabular-nums;white-space:nowrap}
...
.dshsm-scroll-table{overflow:auto;scrollbar-gutter:stable}
.dshsm-scroll-table .dshsm-list thead th{position:sticky;top:0;z-index:1}
.dshsm-import-table-wrap{max-height:min(610px,calc(100vh - 230px))}
.dshsm-metrics-table-wrap{max-height:min(540px,calc(100vh - 270px))}
.dshsm-rowbtns{display:flex;gap:4px;justify-content:flex-start;opacity:0;transition:opacity .12s}
...
.dshsm-dd-current{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dshsm-import-source{flex:0 1 auto;max-width:150px}
.dshsm-import-source .dshsm-dd-btn{max-width:100%;min-width:0;justify-content:flex-start;white-space:nowrap}
...
.dshsm-maximized .dshsm-session-toolbar{display:flex;gap:8px;flex-wrap:nowrap}
.dshsm-maximized .dshsm-session-toolbar .dshsm-session-source,.dshsm-maximized .dshsm-session-toolbar .dshsm-session-workspace,.dshsm-maximized .dshsm-session-toolbar .dshsm-session-status,.dshsm-maximized .dshsm-session-toolbar .dshsm-session-refresh{min-width:0;width:auto}
.dshsm-maximized .dshsm-session-toolbar .dshsm-session-source .dshsm-dd-btn{width:auto;justify-content:flex-start}
.dshsm-maximized .dshsm-session-toolbar .dshsm-session-workspace .dshsm-dd-btn{width:auto;max-width:240px;justify-content:flex-start}
...
.dshsm-rules{display:flex;flex-direction:column;border:1px solid var(--dshsm-border);border-radius:11px;padding:5px 14px 9px;margin-bottom:14px}
.dshsm-rules-row{display:flex;align-items:center;gap:8px 12px;flex-wrap:wrap;padding:8px 0}
.dshsm-rules-row + .dshsm-rules-row{border-top:1px solid var(--dshsm-border)}
.dshsm-rules-label{flex:none;width:36px;font-size:11px;font-weight:600;letter-spacing:.06em;color:var(--dshsm-text-3)}
.dshsm-rules-row .dshsm-rules-run{margin-left:auto}
...
.dshsm-drawer-close{margin-left:auto;flex:none;padding:0;border:0;background:none;appearance:none;display:grid;place-items:center;line-height:1;color:var(--dshsm-text-3);cursor:pointer}
.dshsm-drawer-close:hover{color:var(--dshsm-text)}
.dshsm-drawer-close:focus-visible{outline:2px solid var(--dshsm-accent);outline-offset:4px}
...
@media (prefers-reduced-motion:reduce){
  .dshsm *,.dshsm-overlay-root *,.dshsm-settings-panel-managed{animation:none!important;transition:none!important}
}
`
  ...
  <defs>
    ...
    <symbol id="dshsm-i-expand" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H3v5M16 3h5v5M8 21H3v-5M16 21h5v-5"/></g></symbol>
    <symbol id="dshsm-i-contract" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 8h5V3M21 8h-5V3M3 16h5v5M21 16h-5v5"/></g></symbol>
    ...
    function previewDrawer({ item, preview, onOpen, onArchive }) {
      ...
      const meta = [
        ...
        ['大小', `<span class="dshsm-mono">${esc(formatBytes(preview.sizeBytes))}</span>`],
        ['消息数', `<span class="dshsm-mono">${esc(String(preview.messageCount))}</span>`],
        ['时长', `<span class="dshsm-mono">${esc(formatDuration(preview.durationMs))}</span>`],
        ['工具调用', `<span class="dshsm-mono">${esc(String(preview.toolCalls))}（成功 ${esc(String(preview.toolSuccess))} / 无结果 ${esc(String(preview.toolNoResult))}）</span>`],
    ...
    function Dropdown({ value, onChange, options, ariaLabel, iconOnly, disabled, className }) {
      ...
      return h('div', { className: 'dshsm-dropdown' + (className ? ` ${className}` : ''), ref },
        h('button', {
          ...
          showLabel ? h('span', { className: 'dshsm-dd-current' }, current.label) : null,
    ...
    function InfiniteLoadMarker({ loading, nextCursor, onLoad, label = '会话' }) {
      const markerRef = useRef(null)
      useEffect(() => {
        const node = markerRef.current
        if (!node || !nextCursor || loading) return undefined
        const observer = new IntersectionObserver((entries) => {
          if (entries.some((entry) => entry.isIntersecting)) onLoad(nextCursor)
        }, { rootMargin: '0px 0px 120px 0px' })
        observer.observe(node)
        return () => observer.disconnect()
      }, [loading, nextCursor, onLoad])
      return h('div', { ref: markerRef, className: 'dshsm-load-more', role: 'status', 'aria-live': 'polite' },
        loading
          ? [h(Svg, { key: 'spin', name: 'refresh', size: 14, className: 'dshsm-spin' }), h('span', { key: 'text' }, `正在加载更多${label}…`)]
          : nextCursor ? h('span', null, '继续向下滚动以加载更多') : null)
    }

    /* ================= 会话 tab ================= */

    function SessionsSection({ onTotal, client, close, maximized }) {
      const PAGE_SIZE = 20
      ...
      const load = useCallback((cursor) => {
        const append = typeof cursor === 'string' && cursor.length > 0
        ...
        if (append) params.set('cursor', cursor)
      ...
      const runDeleteOne = useCallback(async (item) => {
        ...
        const ok = await confirmDialog({
          title: '删除会话',
          desc: `将永久删除「<b>${esc(name.length > 40 ? `${name.slice(0, 40)}…` : name)}</b>」，此操作不可恢复。`,
        })
        if (!ok) return
        setBusyId(item.id)
      ...
      const runBatchDelete = useCallback(async () => {
        ...
        const ok = await confirmDialog({
          title: '删除所选会话',
          desc: `将永久删除所选 <b>${selectedCount}</b> 个会话，此操作不可恢复。`,
        })
        if (!ok) return
      ...
      }, [selectedCount, selectedRows, load])
      ...
      const onRowClick = useCallback((e) => {
        ...
        if (loading || loadingMore || busyId) return
        ...
        openPreview(item)
      }, [items, loading, loadingMore, busyId, runDeleteOne, openPreview])

      const table = h('div', {
        ref: tableRef,
        className: 'dshsm-tblwrap' + (loading ? ' dshsm-table-state-loading' : ''),
        'aria-busy': String(loading || loadingMore),
      },
          ...
          h('thead', null,
            h('tr', null,
              h('th', { style: { width: 34 } }, h(SelectAllCheckbox, {
                ...
                disabled: loading || loadingMore || Boolean(busyId) || selectableItems.length === 0,
                onChange: toggleSelectAll, label: '全选当前会话',
              })),
              h('th', null, '标题'),
              h('th', null, '来源'),
              h('th', null, '最后活跃'),
              h('th', { className: 'dshsm-size' }, '大小'),
              h('th', null, '工作区'),
              h('th', null, '状态'),
              h('th', null, '操作'))),
          h('tbody', { onClick: onRowClick },
            items.length === 0
              ? h('tr', null, h('td', { colSpan: 8 }, h('div', { style: { padding: '30px 0' } }),
              ...
              : items.map((item) => {
                ...
                const actions = item.running
                  ...
                  : [
                    h('button', {
                      key: 'arch', type: 'button', className: 'dshsm-btn', 'data-act': item.archived ? 'unarchive' : 'archive', disabled: loading || loadingMore || busy,
                    }, item.archived ? '取消归档' : '归档'),
                    h('button', { key: 'del', type: 'button', className: 'dshsm-btn dshsm-del', 'data-act': 'delete', disabled: loading || loadingMore || busy }, '删除'),
                  ]
                return h('tr', { key: item.id, 'data-id': item.id, className: 'dshsm-clickable' },
                  h('td', null, h('input', {
                    ...
                    disabled: loading || loadingMore || item.running,
                    'data-act': 'check',
                    'data-id': item.id,
                  ...
                  h('td', { className: 'dshsm-size' }, formatBytes(item.sizeBytes)),
                  h('td', { className: 'dshsm-cell-path', title: item.cwd || '未知工作区' }, pathLeaf(item.cwd)),
                  h('td', null, pills.length ? pills : h('span', { style: { color: 'var(--dshsm-text-3)' } }, '—')),
                  h('td', null, h('div', { className: 'dshsm-rowbtns' }, actions)))
              }))),
        h(InfiniteLoadMarker, { loading: loadingMore, nextCursor, onLoad: load }),
        loading ? h('div', {
          ref: loadingRef,
      ...
      return h('section', null,
        h('div', { className: 'dshsm-toolbar dshsm-session-toolbar' },
          ...
          h(Dropdown, { value: source, onChange: setSource, ariaLabel: '来源筛选', options: SOURCE_FILTER_OPTIONS, iconOnly: !maximized, className: 'dshsm-session-source' }),
          h(Dropdown, { value: workspace, onChange: setWorkspace, ariaLabel: '工作区筛选', options: workspaceOptions, iconOnly: !maximized, className: 'dshsm-session-workspace' }),
          h(Dropdown, { value: archived, onChange: setArchived, ariaLabel: '状态筛选', options: ARCHIVED_OPTIONS, className: 'dshsm-session-status' }),
          h('button', { type: 'button', className: 'dshsm-btn dshsm-ghost dshsm-session-refresh', 'aria-label': '刷新会话', title: '刷新', disabled: loading || loadingMore, onClick: () => load() }, h(Svg, { name: 'refresh', size: 15, className: loading ? 'dshsm-spin' : undefined }), '刷新')),
        ...
        selectedCount > 0 ? h('div', { className: 'dshsm-selbar' },
          h('span', { className: 'dshsm-selmeta' }, '已选 ', h('b', null, String(selectedCount)), ' 项'),
          h('span', { style: { flex: 1 } }),
    ...
    function ImportSection({ client }) {
      ...
      const scan = useCallback((nextSource = source, nextWorkspace = workspace, cursor) => {
        const append = typeof cursor === 'string' && cursor.length > 0
        ...
        params.set('limit', String(PAGE_SIZE))
        if (!append && nextWorkspace) params.set('workspace', nextWorkspace)
        if (append) params.set('cursor', cursor)
        if (append && scanId) params.set('scanId', scanId)
        fetchJson(`${API}/scan?${params.toString()}`)
          .then((result) => {
            ...
            setItems((previous) => {
              if (!append) return rows
              const ids = new Set(previous.map((item) => item.sourceSessionId))
              return [...previous, ...rows.filter((item) => !ids.has(item.sourceSessionId))]
            })
            setTotal(result.total != null ? result.total : rows.length)
            setNextCursor(result.nextCursor || null)
      ...
      }, [source, workspace, scanId])

      useEffect(() => { scan(source, workspace) }, []) // 首扫；来源/工作区切换由回调显式重扫

      const loadMore = useCallback((cursor) => scan(source, workspace, cursor), [scan, source, workspace])
      ...
      const isImportSelectable = (item) => !importedIds[item.sourceSessionId] && item.importState !== 'reconciliation-required'
      const selectableItems = items.filter(isImportSelectable)
        ...
        setSelected((prev) => {
          ...
          if (allSelected) {
            ...
            for (const item of items) {
              if (isImportSelectable(item)) next[item.sourceSessionId] = true
            }
          }
          return next
        })
      ...
      const selectedCount = items.filter((item) => selected[item.sourceSessionId] && isImportSelectable(item)).length

      const runImport = useCallback(() => {
        const targets = items
          .filter((item) => selected[item.sourceSessionId] && isImportSelectable(item))
          .map((item) => ({ sourceSessionId: item.sourceSessionId }))
        if (targets.length === 0 || !scanId) return
        setImporting(true)
        setError(null)
        postJson(`${API}/import`, { source, scanId, targets })
          .then((result) => {
            setReport(result)
            setImportedIds((prev) => {
              const next = { ...prev }
      ...
      }, [items, selected, importedIds, source, scanId])

      const emptyText = workspace ? '该工作区没有可导入的历史会话，可切换来源或清除工作区筛选。' : `没有可导入的 ${sourceLabels[source]} 历史会话。`

      const table = h('div', { className: 'dshsm-tblwrap dshsm-scroll-table dshsm-import-table-wrap' },
        h('table', { className: 'dshsm-list dshsm-compact' },
          h('thead', null,
            h('tr', null,
              h('th', { style: { width: 34 } }, h(SelectAllCheckbox, {
                checked: allSelected, mixed: selectedCount > 0 && !allSelected,
                disabled: scanning || loadingMore || importing || picking || selectableItems.length === 0,
          ...
          h('tbody', null,
            items.length === 0
              ? h('tr', null, h('td', { colSpan: 6 }, h('div', { style: { padding: '30px 0' } }),
                h(EmptyState, { title: '没有可导入的会话', sub: emptyText })))
              : items.map((item) => {
                const imported = Boolean(importedIds[item.sourceSessionId])
                const reconciliationRequired = item.importState === 'reconciliation-required'
                return h('tr', { key: item.sourceSessionId, 'data-id': item.sourceSessionId },
                  h('td', null, h('input', {
                    type: 'checkbox',
                    checked: Boolean(selected[item.sourceSessionId]) && !imported,
                    disabled: imported || reconciliationRequired || scanning || loadingMore || importing,
                  ...
                  h('td', { className: 'dshsm-cell-title' },
                    h('div', { className: 'dshsm-t' },
                      ...
                      reconciliationRequired ? h('span', { className: 'dshsm-pill dshsm-fail', style: { marginLeft: 6 } }, '待对账') : null,
                      item.badLines > 0 ? h('span', { className: 'dshsm-pill dshsm-fail', style: { marginLeft: 6 } }, `坏行 ${item.badLines}`) : null),
                    reconciliationRequired && item.dshSessionId
                      ? h('div', { className: 'dshsm-dim', title: item.dshSessionId }, `DSH：${item.dshSessionId}`)
                      : null),
                  h('td', null, h(SourceChip, { source: item.source || source })),
                  h('td', { className: 'dshsm-dim', title: absTime(item.updatedAt) }, relTime(item.updatedAt)),
                  h('td', { className: 'dshsm-size' }, formatBytes(item.sizeBytes)),
                  h('td', { className: 'dshsm-cell-path', title: item.cwd || '未知工作区' }, pathLeaf(item.cwd)))
              }))),
        h(InfiniteLoadMarker, { loading: loadingMore, nextCursor, onLoad: loadMore, label: '导入会话' }))

      return h('section', null,
        h('div', { className: 'dshsm-toolbar' },
          h(Dropdown, { value: source, onChange: changeSource, ariaLabel: '选择导入来源', options: IMPORT_SOURCE_OPTIONS, className: 'dshsm-import-source', disabled: importing || picking }),
          h('button', { type: 'button', className: 'dshsm-btn dshsm-ghost',
            'aria-label': '筛选项目工作区', 'aria-pressed': Boolean(workspace),
            title: workspace ? `工作区：${workspace}` : '选择项目工作区', disabled: picking || importing, onClick: chooseWorkspace,
          }, h(Svg, { name: 'folder', size: 16 }), '筛选项目工作区'),
          workspace ? h('button', { type: 'button', className: 'dshsm-btn dshsm-ghost dshsm-icon-btn',
            ...
            onClick: () => { setWorkspace(''); setSelected({}); setReport(null); scan(source, '') },
          }, h(Svg, { name: 'x', size: 14 })) : null,
          h('button', {
            type: 'button',
            className: 'dshsm-btn dshsm-ghost',
            onClick: () => scan(source, workspace),
            disabled: scanning || loadingMore || importing,
          ...
          h('button', {
            ...
            onClick: runImport,
            disabled: importing || scanning || loadingMore || picking || selectedCount === 0,
          }, h(Svg, { name: 'open', size: 13 }), importing ? '导入中…' : `导入所选（${selectedCount}）`)),
        error ? h('div', { className: 'dshsm-alert dshsm-error' },
          h(Svg, { name: 'warn', size: 15 }),
          h('span', null, String(error)),
          h('button', { type: 'button', className: 'dshsm-alert-act', onClick: () => scan(source, workspace) }, '重试')) : null,
        report ? h('div', { className: 'dshsm-report' },
          h('div', { className: 'dshsm-report-head' }, '导入报告',
            ...
            report.reconciliationRequired ? h('span', { className: 'dshsm-pill dshsm-fail' }, `待对账 ${report.reconciliationRequired}`) : null,
          ...
          h('ul', null, (report.items || []).map((entry, index) =>
            h('li', {
              ...
              className: entry.status === 'failed' || entry.status === 'reconciliation-required' ? 'dshsm-fail' : undefined,
            },
              h('span', { className: 'dshsm-mono' }, entry.sourceSessionId),
              h('span', null, entry.status === 'success' ? '成功' : entry.status === 'skipped' ? '跳过' : entry.status === 'reconciliation-required' ? '待对账' : '失败'),
        ...
        total > items.length && !scanning ? h('div', { className: 'dshsm-hint' }, `已加载 ${items.length} / ${total} 条可导入会话`) : null,
    ...
    function CleanupSection() {
      ...
      return h('section', { className: 'dshsm-cleanwrap' },
        ...
        stats ? h('div', null,
          ...
          h('details', { className: 'dshsm-metrics' },
            ...
            h('div', { className: 'dshsm-metrics-inner' },
              h('div', { className: 'dshsm-tblwrap dshsm-scroll-table dshsm-metrics-table-wrap', style: { border: 'none', borderRadius: 0 } },
                h('table', { className: 'dshsm-list dshsm-compact' },
                  h('thead', null,
                    h('tr', null,
                      h('th', null, '标题'),
                      ...
                      h('th', { className: 'dshsm-num' }, '消息数'),
                      h('th', { className: 'dshsm-num' }, '时长'),
                      h('th', { className: 'dshsm-num' }, '工具调用（成功/无结果）'))),
                  h('tbody', null, (stats.sessions || []).map((s) =>
                    h('tr', { key: s.id },
                      h('td', { className: 'dshsm-cell-title' }, h('div', { className: 'dshsm-t' }, s.title || s.id)),
        ...
        h('div', { className: 'dshsm-rules' },
          h('div', { className: 'dshsm-rules-row' },
            h('span', { className: 'dshsm-rules-label' }, '条件'),
          ...
          h('div', { className: 'dshsm-rules-row' },
            h('span', { className: 'dshsm-rules-label' }, '范围'),
            ...
            h('button', { type: 'button', className: 'dshsm-btn dshsm-primary dshsm-rules-run', onClick: runPreview, disabled: loading }, h(Svg, { name: 'search', size: 13 }), loading ? '生成中…' : '生成预览'))),
        previewBox,
        reportCard)
    }
    ...
    function SessionManagementSection({ client, close }) {
      ...
      useEffect(() => {
        const panel = rootRef.current && rootRef.current.closest('[role="dialog"]')
        panelRef.current = panel
        if (panel) panel.classList.add('dshsm-settings-panel-managed')
        return () => {
          if (panel) panel.classList.remove('dshsm-settings-panel-managed', 'dshsm-settings-panel-maximized')
      ...
      useEffect(() => {
        ...
        if (panel) panel.classList.toggle('dshsm-settings-panel-maximized', maximized)
      }, [maximized])

      return h('div', { className: 'dshsm' + (maximized ? ' dshsm-maximized' : ''), ref: rootRef },
        h('div', { className: 'dshsm-head' },
          h('h2', null, '会话管理'),
          h('button', {
            type: 'button',
            className: 'dshsm-btn dshsm-ghost dshsm-view-toggle',
            'aria-label': maximized ? '还原会话管理窗口' : '最大化会话管理窗口',
            'aria-pressed': String(maximized),
            title: maximized ? '还原窗口' : '最大化窗口',
            onClick: () => setMaximized((value) => !value),
          }, h(Svg, { name: maximized ? 'contract' : 'expand', size: 14 }), maximized ? '还原' : '最大化')),
        ...
        tab === 'sessions' ? h(SessionsSection, { onTotal, client, close, maximized })
    ...
    function apply(ctx) {
      ...
      ), '@nathan110628/dsh-session-management: settings section')
    }
